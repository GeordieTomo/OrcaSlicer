...
//    toggle_line("support_interface_speed", have_support_material && have_support_interface);
    ...
    toggle_line("fuzzy_skin_scale", !is_ripple && fuzzy_skin_noise_type != NoiseType::Classic && has_fuzzy_skin);
    toggle_line("fuzzy_skin_octaves",
                !is_ripple && fuzzy_skin_noise_type != NoiseType::Classic && fuzzy_skin_noise_type != NoiseType::Voronoi && has_fuzzy_skin);
    toggle_line("fuzzy_skin_persistence",
                !is_ripple && (fuzzy_skin_noise_type == NoiseType::Perlin || fuzzy_skin_noise_type == NoiseType::Billow) && has_fuzzy_skin);
    // Ripple-specific layer offset controls — only visible when Zigzag is selected
